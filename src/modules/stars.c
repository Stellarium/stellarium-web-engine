                           ...
                           const char *out)
{
    ...
    char line[1024];
    char sp, de_s;
    int hd, hip, ra_hour, ra_min, de_deg, de_min, de_sec, i, err;
    const int stars_per_tile = 1024;
    double vmag, ra, de, bv, plx = 0, ra_sec;
    double pos[3];
    char buffer[36];
    char id[16];
    (void)err;
    ...
    assert(file);
    for (i = 0; ; i++) {
        if (!gzgets(file, line, ARRAY_SIZE(line))) break;
        catalog_parse_line(BSC_CAT, line, i, &hd, &ra_hour, &ra_min, &ra_sec,
                          &de_s, &de_deg, &de_min, &de_sec, &vmag, &bv, &plx,
                          &sp);
        if (hd == -1) continue;
        assert(ra_hour >= 0 && de_deg >= 0);
        assert(!isnan(vmag));
