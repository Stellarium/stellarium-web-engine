...
const mapColor = function (v) {
  if (!turboSrgbNormalized) {
    turboSrgbNormalized = turboSrgb.map(c => [c[0] / 255, c[1] / 255, c[2] / 255]).map(c => { let m = Math.max(Math.max(c[0], c[1]), c[2]); return [c[0] / m, c[1] / m, c[2] / m] })
  }
  ...
  methods: {
    ...
    refreshGeojsonLiveFilter: function () {
      ...
      that.geojsonObj.filterAll(idx => {
        ...
        that.livefilterData[idx].colorDone = true
        return {
          fill: c,
          stroke: [1, 0, 0, 0],
          visible: true,
