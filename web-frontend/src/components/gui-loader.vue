    ...
    <v-layout column align-center style="width: 100%; height: 100%;">
      ...
      <v-card v-else style="margin: auto;">
        <v-card-title primary-title>
          <div class="headline"><h1>{{ $t('Could not show the Online Star Map') }}</h1></div>
          <div class="headline" style="margin-top: 30px"><v-icon large>error</v-icon> {{ $t('It seems that your browser cannot load Web Assembly!') }}</div>
        </v-card-title>
        <v-card-text>
          <v-layout column align-center style="width: 100%; height: 100%;">
